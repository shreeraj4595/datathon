...
def get_country_data(limit, offset, where):
    ...
    try:
        query = (
            "select a.top_concepts_0, a.title, a.dates_pub, a.dates_accepted, a.dates_online, p.publisher_name_ui, j.journal_name, auth.authors_0_aff_address_country, a.dyson_id ",
            "from Artifacts a "
            "inner join publisher p on a.publisher_id = p.publisher_id inner join Journal j on a.journal_id=j.journal_id "
